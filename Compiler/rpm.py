...
class SwitchNode:
    def __init__(self, switchExpr, caseList, defaultNode):
        # switchExpr é a  expressão que valor será comparado com cada case
        # caseList é a lista de tuplas (caseExprNode, bodyNode)
        # defaultNode é o nó (statement) executado se nenhum case casar
...
class Parser:
    ...
    def statment(self):
        ...
        # - o parser lê o valor do switch, então cada CASE possui uma expressão de comparação
        #   seguida por um único statement (por exemplo PRINT ou atribuição).
        if tok.tipo == TT_ID and tok.valor == 'SWITCH':
            self.avancar()
            switchExpr = self.expr()

            cases = []
            defaultNode = None

            # Leitura de 0 até N cases
            while self.tokenAtual.tipo == TT_ID and self.tokenAtual.valor == 'CASE':
                self.avancar()
                caseExpr = self.expr()
                # O corpo do case é um único statement; isso permite escrever
                # "CASE 1 PRINT 10" ou "CASE 1 x = 5"
                body = self.statment()
                cases.append((caseExpr, body))

            # DEFAULT
            if self.tokenAtual.tipo == TT_ID and self.tokenAtual.valor == 'DEFAULT':
                self.avancar()
                defaultNode = self.statment()

            return SwitchNode(switchExpr, cases, defaultNode)
...
def avaliador(Node):
    ...
    if isinstance(Node, SwitchNode):
        # Avalia a expressão do switch e compara com os cases
        switch_val = avaliador(Node.switchExpr)

        for caseExprNode, bodyNode in Node.caseList:
            case_val = avaliador(caseExprNode)
            # Comparação simples de igualdade (int/float conforme o tipo)
            if switch_val == case_val:
                return avaliador(bodyNode)

        # Se não encontrou case correspondente, executa default se houver
        if Node.defaultNode is not None:
            return avaliador(Node.defaultNode)

        # Se nenhum case e sem default, retorna None
        return None
